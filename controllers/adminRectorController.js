...
const findRector = async (id) => {
  const [rows] = await db.query('SELECT id, rector_id, name, email, mobile, password, role, status, photo, salary, created_at FROM rectors WHERE id = ? LIMIT 1', [id]);
  return rows[0] || null;
};

exports.getRectors = async (req, res) => {
  try {
    const [rectors] = await db.query('SELECT id, rector_id, name, email, mobile, role, status, photo, salary, created_at FROM rectors ORDER BY id DESC');
...
exports.addRector = async (req, res) => {
  try {
    const { rector_id, name, email, mobile, password, role, status, salary } = req.body;
    ...
    const [result] = await db.query('INSERT INTO rectors (rector_id,name,email,mobile,password,role,status,photo,salary) VALUES (?,?,?,?,?,?,?,?,?)', [String(rector_id).trim(), String(name).trim(), cleanEmail, mobile || null, hashed, String(role).trim(), String(status || 'active').toLowerCase(), photo, Number(salary)]);
...
exports.updateRector = async (req, res) => {
  try {
    ...
    await db.query('UPDATE rectors SET rector_id=?,name=?,email=?,mobile=?,password=?,role=?,status=?,photo=?,salary=? WHERE id=?', [String(rector_id).trim(), String(name).trim(), cleanEmail, mobile || null, hashed, String(role).trim(), String(status || 'active').toLowerCase(), photo, Number(salary), req.params.id]);
